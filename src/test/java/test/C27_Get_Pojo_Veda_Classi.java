...
public class C27_Get_Pojo_Veda_Classi extends DummyBaseURL{
    ...
    public void get01(){
        ...
        // 4- Assertion
        // Yukarıdaki response'i pojo formatına döküyoruz
        DummyExpBodyPOJO resPojo = response.as(DummyExpBodyPOJO.class);//Sadece JSonpath direk method olarak geliyor. Diğer bütün class'larda (HashMap veya pojo veya herhangi farklı bir format) as'i kullanmak zorundayız.
        //Buradaki getStatus bizim response üzerinden çağırdığımız getStatus methoduyla aynı değil.Buradaki getStatus (Response Body'deki status) bizim POJO'da hazırladığımız bir getter olarak çalışıyor.
